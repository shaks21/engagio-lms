...
test('Teacher creates 3 breakout rooms, auto-shuffles 6 students, then manually reallocates', async ({ browser }) => {
  ...
  try {
    ...
    // Verify exactly 3 room cards appear (visible only)
    ...
    const manualBtn = tPage.locator('[data-testid="classroom-sidebar"]').getByRole('button', { name: /Manual Allocation/i });
    await expect(manualBtn).toBeVisible();
    await manualBtn.click();
    await tPage.waitForTimeout(500);

    // Unassigned pool should exist
    const unassignedPool = tPage.locator('[data-testid="classroom-sidebar"] [data-testid="unassigned-pool"]').first();
    ...
    const closeBtn = tPage.locator('[data-testid="classroom-sidebar"]').getByRole('button', { name: /Close All Rooms/i });
